def testing():
    ...
    # 첫 번째 공지
    latest_notice_html = soup.select_one(
        'div.list_box > ul > li > div > a.title')
    import slack_api
    from datetime import datetime
    timestamp = datetime.now().strftime('%H:%M:%S %Y-%m-%d')
    if (latest_notice_html):
        slack_api.post_message(
            slack_channel, f"_{timestamp}_ 데몬과 셀레니움이 일하고 있어요!\n")
    else:
        slack_api.post_message(
            slack_channel, f"<!channel> _{timestamp}_ 셀레니움 장애 발생\n")
